...
export default function GetInvolvedPage() {
  return <>
    <section className="bg-gradient-to-br from-uoft-ice to-uoft-pale pt-40 pb-24"><div className="container-shell"><div className="eyebrow">Get involved</div><h1 className="h2 mt-4 max-w-5xl text-uoft-navy">There is more than one way to belong here.</h1><p className="body-lg mt-7 max-w-2xl">Join the chapter, apply to the executive team or take part in mentorship.</p></div></section>
    <section className="section-pad"><div className="container-shell grid gap-6 lg:grid-cols-3">{items.map(([title,text,href,cta])=><article key={title} className="flex min-h-[390px] flex-col rounded-[2rem] border border-slate-200 p-8"><div className="eyebrow">CAGH UTM</div><h2 className="mt-6 text-3xl font-bold text-uoft-navy">{title}</h2><p className="mt-5 leading-7 text-slate-600">{text}</p><a href={href} className="mt-auto inline-flex items-center justify-between rounded-full bg-uoft-navy px-5 py-4 font-bold text-white">{cta}<ArrowUpRight size={17}/></a></article>)}</div><p className="mt-8 text-sm text-slate-500"><strong>Setup note:</strong> the three application URLs are placeholders. Add your real General Member, Executive Application and Mentorship links in <code>src/lib/content.ts</code>.</p></section>
  </>;
}
